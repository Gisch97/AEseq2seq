...
class Seq2Seq(nn.Module):
    ...
    def build_graph(
        ...
        **kwargs
        ...
        self.encode1_out = self.encode1_in * pool_stride_1

        # Bloque 2 (encode2):  
        self.encode2_in = self.encode1_out
        self.encode2_out = self.encode2_in * pool_stride_2
...
def conv_sequence_avg_pooled(input_channels, num_conv=1,  padding=1, pool_stride=2, pool_kernel=2):
    ...
    layers = []
    output_channels = int(input_channels * pool_stride)
...
def transpose_conv_sequence(input_channels, growth_factor=2, num_conv=1, up_kernel=2, padding=1, up_stride=2):
    layers = []
    output_channels = int(input_channels // growth_factor)
    layers.append(nn.ConvTranspose1d(input_channels, output_channels, kernel_size=up_kernel, stride=up_stride, padding=0))
    layers.append(nn.BatchNorm1d(output_channels))
    layers.append(nn.ReLU(inplace=True))
