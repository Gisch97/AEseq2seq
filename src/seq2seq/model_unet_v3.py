...
def conv_sequence(input_channels, growth_factor=2, num_conv=1, kernel_size=3, padding=1, stride=1):
    ...
    for _ in range(num_conv):
        ...
        layers.append(nn.Conv1d(current_channels, next_channels, kernel_size=kernel_size, padding=padding, stride=stride))
        current_channels = next_channels
    
    return nn.Sequential(*layers)

def transpose_conv_sequence(input_channels, growth_factor=2, num_conv=1, kernel_size=3, padding=1, stride=1):
    layers = []
    current_channels = input_channels
    
    for _ in range(num_conv):
        next_channels = int(current_channels / growth_factor)
        layers.append(nn.ConvTranspose1d(current_channels, next_channels, 
