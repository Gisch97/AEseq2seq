...
MODEL_NAME_NO_SKIP="unet_v4_p_no_skip_e1_f0"   
...
echo "Saving hyperparameters:   NUM_CONV1 = ${NUM_CONV1[@]}| 1 ENCODER BLOCKS SKIP CONNS (0 1)"
echo "#Saving hyperparameters:  NUM_CONV1 = ${NUM_CONV1[@]}| 1 ENCODER BLOCKS SKIP CONNS (0 1)" | tee -a "$BASE_OUTPUT_PATH/models.log"
...
for c1 in "${NUM_CONV1[@]}"; do
    ...
    # echo "Executing: num_conv1=$c1; skip=1"
    # # Modify model configuration
    # sed -i \
    #     -e "93s/num_conv1=[0-9e.-]*/num_conv1=$c1/" \
    #     "$MODEL_FILE"

    # save_name="$base_name-1encode-flat1-skip1"
    ...
    echo "Executing: num_conv1=$c1; NO FLAT; skip=0"
    # Modify model configuration
    sed -i \
        -e "93s/num_conv1=[0-9e.-]*/num_conv1=$c1/" \
        "$MODEL_FILE_NO_SKIP"

    save_name="$base_name-1encode-flat0-skip0" 
    echo "save_name: $save_name"
    bash train_test.sh "$BASE_OUTPUT_PATH" "$save_name"
