...
for s1 in "${stride1[@]}"; do 
    for s2 in "${stride2[@]}"; do
        ...
        echo "... Guardando en UNet-kernel3-stride-$s1$s2"
        ...
        sed -i "s/\"run\": \"[^\"]*\"/\"run\": \"UNet-kernel3-stride-$s1$s2\"/" "$global_config"

        # Modificar el archivo train.json
        sed -i "s|\"out_path\": \"[^\"]*\"|\"out_path\": \"results/UNet-kernel3-stride-$s1$s2\"|" "$train_config"
        ...
        sed -i "s|\"model_weights\": \"[^\"]*\"|\"model_weights\": \"results/UNet-kernel3-stride-$s1$s2/weights.pmt\"|" "$test_config"
        sed -i "s|\"out_path\": \"[^\"]*\"|\"out_path\": \"results/UNet-kernel3-stride-$s1$s2/test.csv\"|" "$test_config"
